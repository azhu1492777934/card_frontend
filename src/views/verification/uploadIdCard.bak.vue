<template>
  <div id="canvas-wrapper" class="upload-id-card-wrapper">
    ...
    <!--    <div @click="capture" style="width: 50px;background-color: #c0ff7f">拍摄</div>-->
    ...
    methods: {
      ...
      uploadImgs(step) {
        ...
        if (step >= 0 || (this.previewUrl_0 && this.previewUrl_1 && this.previewUrl_2)) {
          ...
          formData.append('img', this.img_file_0);
          formData.append('test', '211');
          _post('/iot/v1/upload/img1', formData).then(res => {
            this.showLoading = false;
            alert(JSON.stringify(res));
          });

          // _post('/iot/v1/upload/img', formData).then(res => {
          //   this.showLoading = false;
          //   alert(JSON.stringify(res));
          //
          //   // if(res.state === 1){
