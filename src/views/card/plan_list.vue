    ...
    <div class="plan-type-wrap">
      <div class="plan-type-inner-wrap">
        <span
          ...
          :class="{'active_type':index==cur_plan_type_index}"
        >{{item}}</span>
      </div>
    </div>
      ...
      <van-swipe ref="swiperVant" :show-indicators="false" @change="swiperOnChange">
        <van-swipe-item v-for="(item,index) in plan_type" :key="item">
          <ul class="plan-list-wrap">
            <li
              @click="choosePlanClick(inner_index)"
            ...
            >
              <div class="plan-info-wrap">
                ...
                <p class="plan-limited-wrap">
                  ...
                  <span
                    class="limited-num"
                    v-show="inner_item.is_elb_deductible!=0"
                  >可抵扣{{inner_item.max_deductible_elb}}个ELB</span>
                </p>
                <p class="plan-desc">
                  {{ inner_item.describe ?
                  inner_item.describe:inner_item.remark?inner_item.remark:'' }}
                </p>
              </div>
